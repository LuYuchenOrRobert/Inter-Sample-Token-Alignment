...
References:
    * https://github.com/whai362/PVT/blob/v2/classification/main.py

Changes:
    * Added operations:
        - Added necessary imported modules:
            - import pvt_v2_ista2
        - Print the model after it is created.
        - Move saving the model checkpoint next to the evaluation.
        - Save the best model based on test accuracy during training.
        - Added 'ista2_args' when calling 'create_model' function.
        - Scale the warmup_lr and min_lr linearly according to the batch size and the number of GPUs.
    * Modifications:
        - Changed the default value of 'lr' from 5e-4 to 1e-3 according to the experiment setting in pvt_v2 paper.
    * Deleted operations:
        - Removed unused imported modules:
            - import pvt
            - import pvt_v2
            - import collections
        - Removed 'drop_block_rate' when calling 'create_model' function.
"""
...
def main(args):
    ...
    model = create_model(
        ...
        ista2_args=getattr(args, 'ista2_args', {}),
    )
    print(model)
    ...
    linear_scaled_min_lr = args.min_lr * args.batch_size * utils.get_world_size() / 512.0
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir and is_best:
            utils.copy_on_master(checkpoint_path, output_dir / 'model_best.pth')
